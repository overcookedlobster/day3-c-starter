 ...
 *
 * Implement the sum_array function using pointer arithmetic as described in
 * tutorial Step 3.
 *
 ...
 * - Return the sum
 * - In main, test with int nums[4] = {1,2,3,4}; printf("%d", sum_array(nums,
 * 4)); // 10
 *
 * Expected output:
 * Sum of array: 10
...
int sum_array(int *arr, int size) {
  ...
  // Hint: Initialize a pointer to the start of the array, set sum to 0, loop
  // over the size adding the dereferenced pointer value and incrementing the
  // pointer.
...
int main() {
  ...
  int result = sum_array(nums, 4); // Replace 0 with actual call
